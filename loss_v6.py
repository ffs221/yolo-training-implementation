...
def loss_fn(predict_array, labels_array, scope='loss_layer'):
    with tf.variable_scope(scope):
        ...
        # Expand dimension in order to apply cal_iou func
        ...
        # reduce the third dimension in order to use mask on it
        ...
        final_loss_no_object = 0.5 * tf.reduce_sum(total_loss_no_object)  # where 0.5 is lanbda_no_obj
        ############ class loss #################################
        ...
        tiled_g_th_confid_ind = tf.tile(g_th_confid_ind, [1, 1, 1, 20])
        select_loss_classes = tf.multiply(loss_classes, tiled_g_th_confid_ind)
